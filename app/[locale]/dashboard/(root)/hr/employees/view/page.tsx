...
export default function ViewEmployee() {
  ...
  const handleUploadAttachment = useCallback(
    async (id: string | null, files: File[], names: string[], descriptions: string[]) => {
      try {
        if (!id) {
        ...
        const response = await uploadAttachments({ id, formData }).unwrap();
        refetchAttachments();
      } catch (error) {
    ...
    [uploadAttachments, refetchAttachments],
  );

  const handleDownload = (attachment: any) => {
    const link = document.createElement('a');
    link.href = attachment.file; // Use the full file URL from the API response
  ...
  const handleOpenInNewTab = (attachment: any) => {
    window.open(attachment.file, '_blank'); // Use the full file URL instead of file_url
  };
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          ...
          ) : (attachments && attachments.length > 0) || (attachments?.results && attachments.results.length > 0) ? (
            <div className="space-y-4">
              {(attachments.results || attachments).map((attachment: any) => (
                <div
                  key={attachment.id}
      ...
      <AttachmentUploadModal
        ...
        onUpload={(files, names, descriptions) => handleUploadAttachment(id, files, names, descriptions)}
        isLoading={uploading}
      />
    </div>
  );
