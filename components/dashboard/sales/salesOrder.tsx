...
console.log(salesOrders)
  ...
  const getItemNameById = (itemId: string) => {
    const item = existingItems.find((existingItem) => existingItem.id === parseInt(itemId));
    return item ? item.item_name : `Item ${itemId}`; 
  };

  const transformedData = salesOrders?.results.map((order: {
    description: any;
    branch: any;
    id: string; customer: { customer_name: string; }; order_date: string; status: string; total_amount: string; items: { custom_item_name: string; item: string; quantity: string; unit_price: string; discount: string; total: string; }[]; 
}) => ({
    id: order.id,
